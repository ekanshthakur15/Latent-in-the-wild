...
class FingerprintTester:
    ...
    def parse_filename(self, filename):
        ...
        Expected format: Sub003_Left-Hand_MiddleFinger_Wall_Cropped.jpg
        """
        try:
            # Extract subject ID
            ...
            # Extract finger information (Left-Hand_MiddleFinger, etc.)
            ...
            # Extract surface information - look for common surface types
            surface_patterns = [
                r"Wall",
                r"iPad",
                r"Mobile",
            ...
            for pattern in surface_patterns:
                if re.search(pattern, filename, re.IGNORECASE):
                    surface = pattern.lower()
                    break

            # Also try to extract from directory-like patterns in filename
            if surface == "unknown":
                dir_match = re.search(r"\d+-(\w+)_", filename)
                if dir_match:
                    surface = dir_match.group(1).lower()
    ...
    def load_dataset(self):
        """Load and organize dataset with nested folder structure"""
        ...
        for subject_dir in subject_dirs:
            ...
            # Look for surface directories (like 04-Wall_Cropped, 05-iPad_Cropped, etc.)
            surface_dirs = []
            for item in subject_dir.iterdir():
                if item.is_dir():
                    surface_dirs.append(item)

            # Process each surface directory
            for surface_dir in surface_dirs:
                # Check if there are N1/N2 subdirectories
                n_dirs = []
                direct_images = []

                for item in surface_dir.iterdir():
                    if item.is_dir() and item.name in ["N1", "N2"]:
                        n_dirs.append(item)
                ...
                all_image_files = direct_images.copy()

                # Add images from N1/N2 directories
                for n_dir in n_dirs:
                    for ext in ["*.jpg", "*.jpeg", "*.png", "*.bmp"]:
                        all_image_files.extend(n_dir.glob(ext))

                # Process each image file
                for image_file in all_image_files:
                    parsed = self.parse_filename(image_file.name)
                    if parsed:
                        # Add surface directory info
                        parsed["surface_dir"] = surface_dir.name
                        parsed["full_path"] = str(image_file)

                        finger_key = parsed["full_finger_id"]
                        dataset[finger_key]["images"].append(str(image_file))
                        dataset[finger_key]["subject_id"] = parsed["subject_id"]
                        dataset[finger_key]["finger_id"] = parsed["finger_id"]
                        dataset[finger_key]["surfaces"] = dataset[finger_key].get(
                            "surfaces", set()
                        )
        ...
        self.logger.info("Surface distribution:")
